...
# Recoding Pre-Task Preference
df$Choice<-factor(car::recode(as.character(df$Choice), "''=NA; 'Categorize then search'='Organization'; 
...
# Recoding Post-Task Preference 
df$Future.Choice<-factor(car::recode(as.character(df$Future.Choice), "''=NA; 
...
df$Condition<-factor(car::recode(as.character(df$Condition), "''=NA; 'Categorize then search'='Organization';
                     'Search without categorizing'='Search Only'"))
...
df$actualtime.c<-scale(df$ActualTime_Total, center = T, scale = F)
...
* **actualtime.c** is the mean-centered version of the variable **"ActualTime_Total"**

**NOTE 1:** Difference scores calculated as the *Search* condition minius *Organization* condition. Positive scores mean that organization is perceived to take less time than search. Proportion scores are calculated as *(Search-Organization)/Organization*

**NOTE 2:** In the model outputs, categorical variables are followed with a number (e.g., 1 or [1]). This indicates the first level of the factor in question. 

* For **Condition**, **Choice**, and **Future.Choice**, level 1 refers to "Search-Only Strategy"
* For **TaskPhase** in the additional analyses section, level 1 refers to "Pre-Task"
...
```{r}
table(lab$Study, lab$Condition)
```


```{r}
table(lab.ex$Study, lab.ex$Condition)
```
...
### Pre- & Post-Task Time Estimates Predicting Preference
...
tab_model(e1.pre, e1.pre2, e1.post, show.stat = T, digits.p=2, 
          dv.labels = c("Pre-Task", "Pre-Task (N=41)", "Post-Task"))

```

```{r}
tab_model(e1.pre, e1.post, show.stat = T, dv.labels = c("Pre-Task", "Post-Task"))

# tab_model(e1.post,  show.stat = T)

```

<!-- ## E1 Figures -->

<!-- ### Figure 2 (Objective Time and Time Estimates) -->
...
<!-- png(file = "Lego_Figure 2_TimeE1.png", bg = "white", type = c("cairo"), width=2400, height=1500, res=300) -->
...
<!--     strip.background = element_rect(colour="gray70", fill="transparent"), -->
...
tab_model(e2.pre, e2.pre2, e2.post, show.stat = T, digits.p=3)
```

```{r}
tab_model(e2.pre, e2.post, show.stat = T, dv.labels = c("Pre-Task Strategy Preference",
                                                        "Post-Task Strategy Preference"))

tab_model(e2.pre, transform = NULL, show.stat = T)
```
...
Number of prompts given to each individual
```{r}
prompt<-NULL
count<-1
for (i in unique(proge2$ID)){
  sub<-subset(proge2, ID==i)
  prompt[count]<-max(sub$Prompt)
  count<-count+1
}
```
...
tab_model(prog.mod1, show.stat = T)
```

<!-- Power analysis -->

<!-- What is the observed power of the interaction? -->

<!-- ```{r} -->
<!-- e2.prog.mod<-lmer(Response~Progress.c+I(Progress.c^2)*Condition+(1|ID), data = proge2, REML = F) -->
<!-- tab_model(e2.prog.mod, show.stat = T) -->

<!-- powerSim(e2.prog.mod, test=fcompare(Response~Progress.c+I(Progress.c^2)+Condition), nsim=1000) -->


<!-- ``` -->

<!-- Simulate Power given increased effect size: -->

<!-- ```{r} -->
<!-- e2.modsim <- e2.prog.mod -->
<!-- fixef(e2.modsim)['I(Progress.c^2):ConditionOrganization'] <- 30 -->

<!-- e2.sim <- powerSim(e2.modsim, test=fcompare(Response~Progress.c+I(Progress.c^2)+Condition), nsim=1000) -->
<!-- e2.sim -->
...
tab_model(prog.mod4, show.stat = T)
```

```{r}
tab_model(prog.mod4, prog.mod2, show.stat = T, dv.labels = c("Linear Model", "Curvilinear Model"))
...
<!-- ### Figure 5 (Subjective Progress) -->

<!-- ```{r} -->
<!-- png(file = "Lego_Figure 5_TaskProgressE2.png", bg = "white", type = c("cairo"), width=2400, height=1500, res=300) -->

<!-- ggplot(proge2, aes(x=Progress, y=Response, colour=Condition))+ -->
<!--   geom_point(size = 3, alpha = 0.6)+ -->
<!--   scale_colour_manual(values = c("grey20", "grey70"))+ -->
<!--   geom_smooth(formula = y~x, method = lm, se=F, alpha = 0.7)+ -->
<!--   geom_smooth(formula = y ~ x + I(x^2), method = lm, se=F, linetype="dashed")+ -->
...
### Post-Task Preference Across E1 & 2 (Exploratory)

Pooled data across E1 and E2 to examine relative post time estimates as a predictor of post-task preference: 
...
tab_model(post.diff, show.stat = T, digits=2)
...
### Pre- & Post-Task Effort Ratings Predicting Preference
...
```{r}
tab_model(e3.pre, e3.post, show.stat = T, dv.labels = c("Pre-Task Strategy Preference", 
                                                       "Post-Task Strategy Preference"))
```
...
prompt<-NULL
count<-1
for (i in unique(proge3$ID)){
  sub<-subset(proge3, ID==i)
  prompt[count]<-max(sub$Prompt)
  count<-count+1
}
```
...
```{r}
e3.prog.mod<-lmer(PerceivedProgress~Progress.c+I(Progress.c^2)*Condition+(1|ID), data = proge3, REML = F)

e3.obspower<-powerSim(e3.prog.mod, test=fcompare(PerceivedProgress~Progress.c+I(Progress.c^2)+Condition), nsim=1000)
...
```{r}
e3.modsim <- e3.prog.mod
fixef(e3.modsim)['I(Progress.c^2):ConditionOrganization'] <- 0.5

e3.sim <- powerSim(e3.modsim, test=fcompare(PerceivedProgress~Progress.c+I(Progress.c^2)+Condition), nsim=500)
e3.sim
```
...
# prog.mod4<-lmer(PerceivedProgress~Progress.c*Condition+(1|ID), data = proge3, REML = F,
#                 contrasts = list(Condition = "contr.sum"))


tab_model(prog.mod3, show.stat = T, digits=2)
```


```{r}
tab_model(prog.mod3, prog.mod1, show.stat = T, dv.labels = c("Linear Model",
                                                             "Curvilinear Model"))
```
...
<!-- ### Figure 6 (Task Effort Estimates) -->
...
<!-- png(file = "Lego_Figure 6_EffortE3.png", bg = "white", type = c("cairo"), width=2400, height=1500, res=300) -->
...
<!-- ### Figure 7 (Subjective Progress) -->

<!-- ```{r} -->
<!-- png(file = "Lego_Figure 7_TaskProgressE3.png", bg = "white", type = c("cairo"), width=2400, height=1500, res=300) -->
...
# Combined Analysis (Exploratory; E1-3)
...
```{r}
post.mod<-glm(FutureChoice.Num~Choice+Condition+actualtime.c, family="binomial", data=lab.ex,
              contrasts = list(Choice="contr.sum", Condition="contr.sum"))

tab_model(post.mod,  show.stat = T)
```
...
<!-- # png(file = "Lego_Figure 8_CombinedAnalysisE1-3.png", bg = "white", type = c("cairo"), width=2400, height=1500, res=300) -->
...
<!-- ### Pre & Post Task Strategy Selection -->

<!-- ```{r} -->
<!-- # create a long data frame of just pre-task choice -->
<!-- pre.choice<-melt(lab[-c(26)], id=c(1:24, 26:ncol(lab[-c(26)])), -->
<!--                  variable.name = "PrePostChoice", value.name = "Choice.Num") -->
<!-- # create a long data frame of just post-task choice -->
<!-- post.choice<-melt(lab.ex[-c(25)], id=c(1:24, 26:ncol(lab.ex[-c(25)])), -->
<!--                  variable.name = "PrePostChoice", value.name = "Choice.Num")  -->

<!-- lab.choice<-rbind(pre.choice, post.choice) # binding the two dataframes together -->
<!-- lab.choice$PrePostChoice<-factor(recode(lab.choice$PrePostChoice, "'Choice.Num'='Pre-Task'; -->
<!--                                  'FutureChoice.Num'='Post-Task'")) -->
<!-- lab.choice$PrePostChoice<-factor(lab.choice$PrePostChoice, -->
<!--                                  levels(lab.choice$PrePostChoice)[c(2,1)]) -->
...
<!--   return(c(dep.avg=mean(df$Choice.Num, na.rm=T),  -->
<!--            dep.sd=sd(df$Choice.Num, na.rm=T), -->
<!--            dep.count=length(df$Choice.Num)))) -->

<!-- graph2$Study<-car::recode(graph2$Study, "'Study 2'='Experiment 1'; -->
<!--                                           'Study 3'='Experiment 2'; -->
<!--                                           'Study 4'='Experiment 3';") -->

<!-- # png(file = "Lego Manuscript_Figure 3_E1-3.png", bg = "white", type = c("cairo"), width=2400, height=1500, res=300) -->

<!-- # plot -->
<!-- ggplot(graph2, aes(x=PrePostChoice, y=dep.avg), colour="grey70")+  -->
<!--   geom_point(size=4, shape=17, position = position_dodge(0.5))+ -->
<!--   geom_errorbar(aes(ymax=dep.avg+dep.sd*qt(0.975,df=dep.count-1)/sqrt(dep.count),  -->
<!--                     ymin=dep.avg-dep.sd*qt(0.975,df=dep.count-1)/sqrt(dep.count)), -->
<!--                 width=0, position = position_dodge(0.5))+ -->
...
<!-- ```{r} -->
<!-- lab.est<- lab.choice %>%  -->
<!--   pivot_longer(cols = c(PreTimeDiff, PostTimeDiff), names_to = "PrePostTime", values_to = "TimeDiff") %>% -->
<!--   pivot_longer(cols = c(PreEffDiff, PostEffDiff), names_to = "PrePostEff", values_to = "EffDiff")  -->

<!-- # recoding values -->
<!-- lab.est$PrePostTime<-factor(car::recode(lab.est$PrePostTime, "'PreTimeDiff'='Pre-Task'; -->
<!--                                  'PostTimeDiff'='Post-Task'")) -->
<!-- lab.est$PrePostTime<-factor(lab.est$PrePostTime, -->
<!--                                  levels(lab.est$PrePostTime)[c(2,1)]) -->

<!-- # recoding values -->
<!-- lab.est$PrePostEff<-factor(car::recode(lab.est$PrePostEff, "'PreEffDiff'='Pre-Task'; -->
<!--                                  'PostEffDiff'='Post-Task'")) -->
<!-- lab.est$PrePostEff<-factor(lab.est$PrePostEff, -->
<!--                                  levels(lab.est$PrePostEff)[c(2,1)]) -->

<!-- lab.est$Choice.Num<-factor(car::recode(lab.est$Choice.Num, '1="Organization"; 0="Search-Only"')) -->
<!-- lab.est$Choice.Num<-factor(lab.est$Choice.Num, -->
<!--                                  levels(lab.est$Choice.Num)[c(2,1)]) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- # Exp 1 -->
<!-- graph2<-ddply(lab.est[lab.est$Study=="In Lab E1",], c("PrePostTime", "Choice.Num" ), function(df)    -->
<!--   return(c(dep.avg=mean(df$TimeDiff, na.rm=T),  -->
<!--            dep.sd=sd(df$TimeDiff, na.rm=T), -->
<!--            dep.count=length(df$TimeDiff)))) -->

<!-- # plot -->
<!-- e1<-ggplot(graph2, aes(x=PrePostTime, y=dep.avg, colour=Choice.Num))+  -->
<!--   geom_point(size=4, shape=17, position = position_dodge(0.5))+ -->
<!--   geom_errorbar(aes(ymax=dep.avg+dep.sd*qt(0.975,df=dep.count-1)/sqrt(dep.count),  -->
<!--                     ymin=dep.avg-dep.sd*qt(0.975,df=dep.count-1)/sqrt(dep.count)), -->
<!--                 width=0, position = position_dodge(0.5))+ -->
...
<!--     ,legend.position="none" -->
<!--   )+ -->
<!--   scale_colour_manual("Strategy\nPreference", values=c("grey20", "grey70"))+ -->
<!--   geom_hline(yintercept = 0, linetype="dotted")+ -->
<!--   scale_x_discrete("")+ -->
<!--   scale_y_continuous("Relative Estimated Task Time\n(Search - Organization)", breaks=seq(-10, 10, 2))+ -->
...
<!-- ```{r} -->
<!-- # Exp 2 -->
<!-- graph2<-ddply(lab.est[lab.est$Study=="In Lab E2",], c("PrePostTime", "Choice.Num" ), function(df)    -->
<!--   return(c(dep.avg=mean(df$TimeDiff, na.rm=T),  -->
<!--            dep.sd=sd(df$TimeDiff, na.rm=T), -->
<!--            dep.count=length(df$TimeDiff)))) -->

<!-- # plot -->
<!-- e2<-ggplot(graph2, aes(x=PrePostTime, y=dep.avg, colour=Choice.Num))+  -->
<!--   geom_point(size=4, shape=17, position = position_dodge(0.5))+ -->
<!--   geom_errorbar(aes(ymax=dep.avg+dep.sd*qt(0.975,df=dep.count-1)/sqrt(dep.count),  -->
<!--                     ymin=dep.avg-dep.sd*qt(0.975,df=dep.count-1)/sqrt(dep.count)), -->
<!--                 width=0, position = position_dodge(0.5))+ -->
...
<!-- ```{r} -->
<!-- # Exp 3 -->
<!-- graph2<-ddply(lab.est[lab.est$Study=="In Lab E3",], c("PrePostEff", "Choice.Num" ), function(df)    -->
<!--   return(c(dep.avg=mean(df$EffDiff, na.rm=T),  -->
<!--            dep.sd=sd(df$EffDiff, na.rm=T), -->
<!--            dep.count=length(df$EffDiff)))) -->

<!-- # plot -->
<!-- e3<-ggplot(graph2, aes(x=PrePostEff, y=dep.avg, colour=Choice.Num))+  -->
<!--   geom_point(size=4, shape=17, position = position_dodge(0.5))+ -->
<!--   geom_errorbar(aes(ymax=dep.avg+dep.sd*qt(0.975,df=dep.count-1)/sqrt(dep.count),  -->
<!--                     ymin=dep.avg-dep.sd*qt(0.975,df=dep.count-1)/sqrt(dep.count)), -->
<!--                 width=0, position = position_dodge(0.5))+ -->
...
<!--     legend.background = element_rect(linetype="solid", fill = "transparent", -->
<!--                                   colour ="grey70") -->
<!--   )+ -->
<!--   scale_colour_manual("Strategy\nPreference", values=c("grey20", "grey70"))+ -->
<!--   geom_hline(yintercept = 0, linetype="dotted")+ -->
<!--   scale_x_discrete("")+ -->
<!--   scale_y_continuous("Relative Perceived Effort\n(Search - Organization)", breaks=seq(-10, 10, 0.5))+ -->
...
<!-- png(file = "Lego_Figure Extra_CombinedDifference Score.png", bg = "white", type = c("cairo"), width=3600, height=1500, res=300) -->

<!-- plot_grid(e1, e2, e3, nrow=1, labels = NULL) -->
<!-- ``` -->
...
### 1. Pre/Post as a factor

```{r}
df.pre<-df[c(1:24, 25,27,29)]
names(df.pre)[c(25:27)]<-c("Preference", "Time", "Effort")
df.pre$TaskPhase<-"Pre-Task"

df.post<-df[c(1:24, 26,28,30)]
names(df.post)[c(25:27)]<-c("Preference", "Time", "Effort")
df.post$TaskPhase<-"Post-Task"

df.new<-rbind(df.pre, df.post)
df.new$TaskPhase<-factor(df.new$TaskPhase)
df.new$TaskPhase<-factor(df.new$TaskPhase, levels(df.new$TaskPhase)[c(2,1)])

```


```{r}
e1.time1<-glmer(Preference ~ TaskPhase+Time+(1|ID), family="binomial", data=df.new[df.new$Study=="In Lab E1",],
               contrasts = list(TaskPhase = "contr.sum"))

e1.time2<-glmer(Preference ~ TaskPhase+Time+(TaskPhase|ID), family="binomial", data=df.new[df.new$Study=="In Lab E1",],
               contrasts = list(TaskPhase = "contr.sum"))

anova(e1.time1, e1.time2)

# did not improve model fit, going with model 1

e2.time1<-glmer(Preference ~ TaskPhase+Time+(1|ID), family="binomial", data=df.new[df.new$Study=="In Lab E2",],
               contrasts = list(TaskPhase = "contr.sum"))

# e2.time2<-glmer(Preference ~ TaskPhase+Time+(TaskPhase|ID), family="binomial", data=df.new[df.new$Study=="In Lab E2",],
#                contrasts = list(TaskPhase = "contr.sum"))

# model with slope could not be estimated given the limited number of observations


e3.effort1<-glmer(Preference ~ TaskPhase+Effort+(1|ID), family="binomial", data=df.new[df.new$Study=="In Lab E3",],
               contrasts = list(TaskPhase = "contr.sum"))

# e3.effort2<-glmer(Preference ~ TaskPhase+Effort+(TaskPhase|ID), family="binomial", data=df.new[df.new$Study=="In Lab E3",],
#                contrasts = list(TaskPhase = "contr.sum"))

# model with slope could not be estimated given the limited number of observations

tab_model(e1.time1,  show.stat = T, dv.labels = "E1 Pre- vs Post-Task Strategy Preference")

tab_model(e2.time1,  show.stat = T, dv.labels = "E2 Pre- vs Post-Task Strategy Preference")

tab_model(e3.effort1,  show.stat = T, dv.labels = "E3 Pre- vs Post-Task Strategy Preference")
...
### 2. Assigned Task and Task Time as additional predictors of Post-Task Strategy Preference

#### Post-Task


```{r}
e1.post<-glm(FutureChoice.Num ~ PostTimeDiff, family="binomial", 
             data=e1.ex)

e2.post<-glm(FutureChoice.Num ~ PostTimeDiff, family="binomial", 
              data=e2.ex)

e3.post<-glm(FutureChoice.Num ~ PostEffDiff, family="binomial", 
              data=e3.ex)

# tab_model(e1.post, e2.post, e3.post, show.stat = T,
#           dv.labels = c("E1", "E2", "E3"))
```

```{r}
e1.post2<-glm(FutureChoice.Num ~ PostTimeDiff+Condition+actualtime.c, family="binomial", 
              data=e1.ex, contrasts = list(Condition="contr.sum"))

e2.post2<-glm(FutureChoice.Num ~ PostTimeDiff+Condition+actualtime.c, family="binomial", 
              data=e2.ex, contrasts = list(Condition="contr.sum"))

e3.post2<-glm(FutureChoice.Num ~ PostEffDiff+Condition+actualtime.c, family="binomial", 
              data=e3.ex, contrasts = list(Condition="contr.sum"))

# tab_model(e1.post2, e2.post2, e3.post2,  show.stat = T,
#           dv.labels = c("E1", "E2", "E3"))

```

```{r}
anova(e1.post, e1.post2, test = "LR")
```

```{r}
anova(e2.post, e2.post2, test = "LR")
```

```{r}
anova(e3.post, e3.post2, test = "LR")
```


```{r}
tab_model(e1.post, e1.post2, show.stat = T, dv.labels = c("Original Model", "Model With Additional Predictors"))
```

```{r}
tab_model(e2.post, e2.post2, show.stat = T, dv.labels = c("Original Model", "Model With Additional Predictors"))
```

```{r}
tab_model(e3.post, e3.post2, show.stat = T, dv.labels = c("Original Model", "Model With Additional Predictors"))
```
